...
data_Age$AgeWeights_Mean = as.numeric(Data_Mat$AgeWeights.All.NoMedialWall.RandomCV);
...
ggplot() +
         ...
         scale_x_continuous(limits = c(-0.000001, 0.048), breaks = c(0, 0.02, 0.04)) +
         scale_y_continuous(limits = c(-0.000001, 0.060), breaks = c(0, 0.02, 0.04, 0.06));
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/AgePredictionWeights_Variability_RandomCV.tiff', width = 17, height = 15, dpi = 600, units = "cm");

# Cognition prediction weights (random CV) vs. atlas variability
data_Cognition = data.frame(Variability_Data = as.numeric(t(Data_Mat$VariabilityLoading.17SystemMean.All.NoMedialWall)));
data_Cognition$CognitionWeights_Mean = as.numeric(Data_Mat$CognitionWeights.All.NoMedialWall.RandomCV);
...
ggplot() +
        geom_hex(data = subset(data_hex, count > 10), aes(x, y, fill = count), stat = "identity") +
        scale_fill_gradientn(colours = myPalette, breaks = c(80, 160)) +
...
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/EFPredictionWeights_Variability_RandomCV.tiff', width = 16, height = 15, dpi = 600, units = "cm");

# Significance
# AgeWeights (Random CV) vs. variability 
tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall.RandomCV),
...
for (i in c(1:1000))
{
  ...
  Non100_Index = which(Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i,] != 100);
  tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall.RandomCV[Non100_Index]),
                      as.numeric(Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i, Non100_Index]), method = "pearson");
...
PermutationData$Line_y = as.numeric(seq(0,149,length.out=1000));
ggplot(PermutationData) +
    ...
    scale_x_continuous(limits = c(-0.4, 0.60), breaks = c(-0.4, 0, 0.4), labels = c('-0.4', '0', '0.4'))
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/SpinTestDensity_AgeWeights_Variability_RandomCV.tiff', width = 17, height = 15, dpi = 600, units = "cm");

# CognitionWeights (RandomCV) vs. variability
tmp_data = cor.test(as.numeric(Data_Mat$CognitionWeights.All.NoMedialWall.RandomCV),
                    as.numeric(Data_Mat$VariabilityLoading.17SystemMean.All.NoMedialWall), method = "pearson");
...
for (i in c(1:1000))
{
  ...
  # Remove medial wall and low SNR regions in the permuted data
  Non100_Index = which(Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i,] != 100);
  tmp_data = cor.test(as.numeric(Data_Mat$CognitionWeights.All.NoMedialWall.RandomCV[Non100_Index]),
                      as.numeric(Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i, Non100_Index]), method = "pearson");
...
PermutationData$Line_x = as.numeric(matrix(Actual_Corr_Variability_CognitionWeights, 1, 1000));
...
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/SpinTestDensity_CognitionWeights_Variability_RandomCV.tiff', width = 17, height = 15, dpi = 600, units = "cm");
