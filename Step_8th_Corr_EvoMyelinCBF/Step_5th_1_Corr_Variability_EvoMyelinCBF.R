...
for (i in c(1:1000))
{
  ...
  Non100_Index = which(Data_Mat$VariabilityLoading.17SystemMean.Perm.rh.NoMedialWall[i,] != 100);
  tmp_data = cor.test(as.numeric(Data_Mat$VariabilityLoading.17SystemMean.Perm.rh.NoMedialWall[i, Non100_Index]),
...
{
  print(i);
  VariabilityLoading_tmp = Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i,Index];
  Myelin_tmp = Data_Mat$Myelin.All.NoMedialWall[Index];
  # Remove medial wall ind low SNR regions in the permuted data
  Non100_Index = which(VariabilityLoading_tmp != 100);
  tmp_data = cor.test(as.numeric(VariabilityLoading_tmp[Non100_Index]), 
                      as.numeric(Myelin_tmp[Non100_Index]), method = "pearson");
...
PermutationData$Line_y = as.numeric(seq(0,118,length.out=1000));
ggplot(PermutationData) +
    geom_histogram(aes(x = x), bins = 30, color = "#999999", fill = "#999999") +
    #geom_point(aes(x = Actual_Corr, y = 0), size=3) +
    geom_line(aes(x = Line_x, y = Line_y, group = 1), size = 1, color = 'red', linetype = "dashed") +
...
for (i in c(1:1000))
{
  print(i)
  VariabilityLoading_tmp = Data_Mat$VariabilityLoading.17SystemMean.Perm.All.NoMedialWall[i,Index];
  Myelin_tmp = Data_Mat$MeanCBF.All.NoMedialWall[Index];
  # Remove medial wall ind low SNR regions in the permuted data
  Non100_Index = which(VariabilityLoading_tmp != 100);
  tmp_data = cor.test(as.numeric(VariabilityLoading_tmp[Non100_Index]),
                      as.numeric(Myelin_tmp[Non100_Index]), method = "pearson");
...
PermutationData$Line_y = as.numeric(seq(0,128,length.out=1000));
ggplot(PermutationData) +
    geom_histogram(aes(x = x), bins = 30, color = "#999999", fill = "#999999") +
    #geom_point(aes(x = Actual_Corr, y = 0), size=3) +
    geom_line(aes(x = Line_x, y = Line_y, group = 1), size = 1, color = 'red', linetype = "dashed") +
