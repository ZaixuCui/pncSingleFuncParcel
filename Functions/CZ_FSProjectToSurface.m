...
function CZ_FSProjectToSurface(rawDir, targDir, BBLID, ScanID, StructuralFSFolder)
...
for j = 1:2
    ...
    cmd = ['export SUBJECTS_DIR="' StructuralFSFolder '"; mri_vol2surf --mov ' rawDir '/' BBLID_Str '/*' ScanID_Str '/regress/*residualised.nii.gz --reg ' targDir '/' BBLID_Str '/coreg/fs_ep2struct_fsl.dat --hemi ' hemi{j} ' --o ' targDir '/' BBLID_Str '/surf/' hemi{j} '.sm6.residualised.mgh --projfrac 0.5 --interp trilinear --noreshape --surf-fwhm 6 >> ' targDir '/' BBLID_Str '/surf/' BBLID_Str '_projection.log 2>&1'];
    system(cmd);
