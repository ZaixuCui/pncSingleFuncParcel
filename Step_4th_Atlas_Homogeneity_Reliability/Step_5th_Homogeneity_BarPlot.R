...
Homogeneity_Hongming = readMat(paste0(Homogeneity_Folder, '/Homogeneity_Hongming_AllSubjects.mat'));
...
tmp$Label <- factor(tmp$Label, levels = c(1:4), labels = c("Indiv. NMF", "Indiv. MS-HBM", "Group NMF", "Yeo 17"));
...
Fig <- Fig + theme(axis.text.x = element_text(size = 28, colour = "black"), axis.text.y = element_text(size = 32, colour = "black"), axis.title = element_text(size = 32));
...
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/Homogeneity_hist.tiff', width = 17, height = 21.5, dpi = 300, units = "cm");
