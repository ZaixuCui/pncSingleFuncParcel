...
# Age
data$NumOfSigVertices_Univariate_Age = as.numeric(matrix(0, 1, 17));
# EF
...
for (i in c(1:17)) {
    # Age
    tmp_Data = readMat(paste0(AgeEffect_Univariate_Folder, '/AgeEffect_AtlasLoading_17_Network_', as.character(i), '.mat'));
    data$NumOfSigVertices_Univariate_Age[i] = length(which(tmp_Data$Gam.Z.FDR.Sig.Vector.All != 0));
    #data$NumOfSigVertices_Univariate_Age[i] = length(which(tmp_Data$Gam.P.Vector.All < 0.05 & tmp_Data$Gam.Z.Vector.All!= 0));
    # EF
...
# Age
cor.test(data$NetworkVariability, data$NumOfSigVertices_Univariate_Age);
# EF
cor.test(data$NetworkVariability, data$NumOfSigVertices_Univariate_EF);

# Age
ggplot(data, aes(NetworkVariability, NumOfSigVertices_Univariate_Age)) + 
    geom_point() + 
    geom_smooth(method = lm) +
    theme_classic() +
    theme(axis.text=element_text(size = 30, color = 'black'), axis.title = element_text(size = 30)) +
    scale_y_continuous(limits = c(0, 2000), breaks = c(0, 500, 1500, 2000), label = c("0", "500", "1500", "2000")) +
...
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/NetworkVariability_AgeEffects_Univariate.tiff', width = 19, height = 15, dpi = 600, units = "cm");
