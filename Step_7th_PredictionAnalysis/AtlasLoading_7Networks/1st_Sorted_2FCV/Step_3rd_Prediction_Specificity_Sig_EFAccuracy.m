...
EFPrediction_ResFolder = [PredictionFolder '/AtlasLoading_7Networks/2Fold_Sort_EFAccuracy'];
...
[ParCorr_Actual_Fold0, ~] = partialcorr(Prediction_Fold0.Predict_Score', BehaviorScore_Tested_Fold0, ...
        [double(Age_Fold0) double(Sex_Fold0) double(Motion_Fold0)]);
[ParCorr_Actual_Fold1, ~] = partialcorr(Prediction_Fold1.Predict_Score', BehaviorScore_Tested_Fold1, ...
        [double(Age_Fold1) double(Sex_Fold1) double(Motion_Fold1)]);
...
%% Significance
EFAccuracyPrediction_PermutationFolder = [PredictionFolder '/AtlasLoading_7Networks/2Fold_Sort_Permutation_EFAccuracy'];
% Fold 0
...
for i = 1:1000
  ...
  ParCorr_Rand_Fold0(i) = partialcorr(tmp.Predict_Score', BehaviorScore_Tested_Fold0, ...
         [double(Age_Fold0) double(Sex_Fold0) double(Motion_Fold0)]);
...
for i = 1:1000
  ...
  ParCorr_Rand_Fold1(i) = partialcorr(tmp.Predict_Score', BehaviorScore_Tested_Fold1, ...
         [double(Age_Fold1) double(Sex_Fold1) double(Motion_Fold1)]);
