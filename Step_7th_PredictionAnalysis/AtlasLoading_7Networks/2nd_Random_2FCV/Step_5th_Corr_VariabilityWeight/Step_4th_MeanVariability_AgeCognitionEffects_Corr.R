...
for (i in c(1:1000))
{
  print(i);
  tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall),
                      as.numeric(Data_Mat$VariabilityLoading.7SystemMean.Perm.All.NoMedialWall[i,]), method = "pearson");
...
PermutationData$Line_y = as.numeric(seq(0,122,length.out=1000));
...
PermutationData$Line_y = as.numeric(seq(0,126,length.out=1000));
ggplot(PermutationData) +
    geom_histogram(aes(x = x), bins = 30, color = "#999999", fill = "#999999") +
    #geom_point(aes(x = Actual_Corr, y = 0), size=3) +
    geom_line(aes(x = Line_x, y = Line_y, group = 1), size = 1, color = 'red', linetype = "dashed") +
...
{
  print(i);
  tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.17SystemMean.All.NoMedialWall),
                      as.numeric(Data_Mat$AgeWeights.Perm.All.NoMedialWall[i,]), method = "pearson");
...
{
  print(i);
  tmp_data = cor.test(as.numeric(Data_Mat$EFWeights.17SystemMean.All.NoMedialWall),
                      as.numeric(Data_Mat$EFWeights.Perm.All.NoMedialWall[i,]), method = "pearson");
  Perm_Corr_7Networks_17Networks[i] = tmp_data$estimate;
}
P_Value = length(which(Perm_Corr_7Networks_17Networks >= Actual_Corr_7Networks_17Networks)) / 1000;
print(paste0('P value (variability vs. age prediction weights): ', as.character(P_Value)));
