...
Folder = '/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/PredictionAnalysis/AtlasLoading_7Networks/Corr_Variability_AgePredictionWeights';
...
data_Age = data.frame(Variability_Data = as.numeric(t(Data_Mat$VariabilityLoading.7SystemMean.All.NoMedialWall)));
...
ggplot() +
         ...
         scale_fill_gradientn(colours = myPalette, breaks = c(200, 400)) +
         ...
         scale_x_continuous(limits = c(-0.000001, 0.100), breaks = c(0, 0.04, 0.08)) + 
         scale_y_continuous(limits = c(-0.000001, 0.05), breaks = c(0, 0.02, 0.04));
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/AgePredictionWeights_Variability_7Networks.tiff', width = 17, height = 15, dpi = 600, units = "cm");

# Correlation between EF prediction weights and atlas variability
data_EF = data.frame(Variability_Data = as.numeric(t(Data_Mat$VariabilityLoading.7SystemMean.All.NoMedialWall)));
data_EF$EFWeights_Mean = as.numeric(Data_Mat$EFWeights.All.NoMedialWall);
cor.test(data_EF$EFWeights_Mean, data_EF$Variability_Data, method = "pearson");
...
ggplot() +
         ...
         theme(legend.justification = c(1, 1), legend.position = c(1, 1)) +
         scale_x_continuous(limits = c(-0.000001, 0.100), breaks = c(0, 0.04, 0.08)) +
         scale_y_continuous(limits = c(-0.000001, 0.035), breaks = c(0, 0.01, 0.02, 0.03));
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/EFPredictionWeights_Variability_7Networks.tiff', width = 17, height = 15, dpi = 600, units = "cm");

# Significance
# AgeWeights vs. variability
tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall),
                    as.numeric(Data_Mat$VariabilityLoading.7SystemMean.All.NoMedialWall), method = "pearson");
...
for (i in c(1:1000))
{
  ...
  Non100_Index = which(Data_Mat$VariabilityLoading.7SystemMean.Perm.All.NoMedialWall[i,] != 100);
  tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall[Non100_Index]),
                      as.numeric(Data_Mat$VariabilityLoading.7SystemMean.Perm.All.NoMedialWall[i,Non100_Index]), method = "pearson");
...
PermutationData$Line_y = as.numeric(seq(0,119,length.out=1000));
ggplot(PermutationData) + 
    ...
    scale_x_continuous(limits = c(-0.4, 0.6), breaks = c(-0.4, 0, 0.4), labels = c('-0.4', '0', '0.4'))
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/SpinTestDensity_AgeWeights_Variability_7Networks.tiff', width = 17, height = 15, dpi = 600, units = "cm");
...
PermutationData$Line_y = as.numeric(seq(0,109,length.out=1000));
ggplot(PermutationData) +
    geom_histogram(aes(x = x), bins = 30, color = "#999999", fill = "#999999") +
    #geom_point(aes(x = Actual_Corr, y = 0), size=3) +
    geom_line(aes(x = Line_x, y = Line_y, group = 1), size = 1, color = 'red', linetype = "dashed") +
    ...
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(limits = c(-0.4, 0.6), breaks = c(-0.4, 0, 0.4), labels = c('-0.4', '0', '0.4'))
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/SpinTestDensity_EFWeights_Variability_7Networks.tiff', width = 17, height = 15, dpi = 600, units = "cm");

# AgeWeights 17 networks vs. ageweights 7 networks
tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.All.NoMedialWall),
                    as.numeric(Data_Mat$AgeWeights.17SystemMean.All.NoMedialWall), method = "pearson");
Actual_Corr_7Networks_17Networks = tmp_data$estimate;
Perm_Corr_7Networks_17Networks = matrix(0, 1, 1000);
for (i in c(1:1000))
{
  print(i);
  # Remove medial wall and low SNR regions in the permuted data
  Non100_Index = which(Data_Mat$AgeWeights.Perm.All.NoMedialWall[i,] != 100);
  tmp_data = cor.test(as.numeric(Data_Mat$AgeWeights.17SystemMean.All.NoMedialWall[Non100_Index]),
...
tmp_data = cor.test(as.numeric(Data_Mat$EFWeights.All.NoMedialWall),
                    as.numeric(Data_Mat$EFWeights.17SystemMean.All.NoMedialWall), method = "pearson");
Actual_Corr_7Networks_17Networks = tmp_data$estimate;
Perm_Corr_7Networks_17Networks = matrix(0, 1, 1000);
for (i in c(1:1000))
{
  ...
  tmp_data = cor.test(as.numeric(Data_Mat$EFWeights.17SystemMean.All.NoMedialWall[Non100_Index]),
