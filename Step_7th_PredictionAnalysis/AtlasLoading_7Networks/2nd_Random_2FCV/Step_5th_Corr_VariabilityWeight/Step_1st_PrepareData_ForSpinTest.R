...
VariabilityLoading_7SystemMean_lh_CSV = data.frame(VariabilityLoading_lh = 
                                  t(VariabilityLoading_7SystemMean_Mat$VariabilityLoading.Median.7SystemMean.lh));
...
VariabilityLoading_7SystemMean_rh_CSV = data.frame(VariabilityLoading_rh = 
                                  t(VariabilityLoading_7SystemMean_Mat$VariabilityLoading.Median.7SystemMean.rh));
...
w_Brain_Age_Abs_Sum_lh_CSV = data.frame(w_Brain_Age_Abs_Sum_lh = 
                                  w_Brain_Age_Abs_Sum_Mat$w.Brain.Age.Abs.sum.lh);
write.table(w_Brain_Age_Abs_Sum_lh_CSV, paste0(SpinTest_Folder, '/w_Brain_Age_Abs_Sum_lh.csv'), row.names = FALSE, col.names = FALSE);
w_Brain_Age_Abs_Sum_rh_CSV = data.frame(w_Brain_Age_Abs_Sum_rh = 
                                  w_Brain_Age_Abs_Sum_Mat$w.Brain.Age.Abs.sum.rh);
write.table(w_Brain_Age_Abs_Sum_rh_CSV, paste0(SpinTest_Folder, '/w_Brain_Age_Abs_Sum_rh.csv'), row.names = FALSE, col.names = FALSE);

w_Brain_EFAccuracy_Abs_Sum_lh_CSV = data.frame(w_Brain_EFAccuracy_Abs_Sum_lh = 
                                  w_Brain_EFAccuracy_Abs_Sum_Mat$w.Brain.EFAccuracy.Abs.sum.lh);
...
w_Brain_EFAccuracy_Abs_Sum_rh_CSV = data.frame(w_Brain_EFAccuracy_Abs_Sum_rh = 
                                  w_Brain_EFAccuracy_Abs_Sum_Mat$w.Brain.EFAccuracy.Abs.sum.rh);
