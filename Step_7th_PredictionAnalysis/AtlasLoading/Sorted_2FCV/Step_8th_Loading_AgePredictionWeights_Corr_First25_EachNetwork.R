...
w_Brain_Age_Abs_FirstPercent_All = matrix(0, 1, 18715*17);
w_Brain_Age_Abs_FirstPercent_All[NonZeroIndex] = w_Brain_Age_Abs_FirstPercent;

FigureFolder = '/data/jux/BBL/projects/pncSingleFuncParcel/Replication/results/Figures/Loading_AgeWeight_First25Percent';
...
for (i in c(1:17)){
    ...
    # plot histogram for network 17
      ...
      PermutationData$Line_y = as.numeric(seq(0,103,length.out=1000));
      ggplot(PermutationData) +
          ...
          scale_x_continuous(limits = c(-0.25, 0.25), breaks = c(-0.2, 0, 0.2), labels = c('-0.2', '0', '0.2'))     
      ggsave(paste0(FigureFolder,'/PearCorr_Loading_AgeWeightAbs_Network17.pdf'), width = 17, height = 15, dpi = 600, units = "cm");
    }
}
