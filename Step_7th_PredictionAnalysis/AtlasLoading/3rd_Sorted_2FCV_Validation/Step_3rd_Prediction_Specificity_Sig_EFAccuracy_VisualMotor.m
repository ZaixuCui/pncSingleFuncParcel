...
EFPrediction_Association = [PredictionFolder ...
    '/AtlasLoading_Validation/2Fold_Sort_EFAccuracy_Association'];
Prediction_Fold0_Association = load([EFPrediction_Association '/Fold_0_Score.mat']);
Prediction_Fold1_Association = load([EFPrediction_Association '/Fold_1_Score.mat']);
...
[ParCorr_Actual_Fold0, ~] = partialcorr(Prediction_Fold0.Predict_Score', EF_Fold0, ...
        [double(Age_Fold0) double(Sex_Fold0) double(Motion_Fold0) double(Prediction_Fold0_Association.Predict_Score')]);
[ParCorr_Actual_Fold1, ~] = partialcorr(Prediction_Fold1.Predict_Score', EF_Fold1, ...
        [double(Age_Fold1) double(Sex_Fold1) double(Motion_Fold1) double(Prediction_Fold1_Association.Predict_Score')]);
...
EFAccuracyPrediction_PermutationFolder_Association = [PredictionFolder ...
...
Permutation_Fold0_Cell_Association = g_ls([EFAccuracyPrediction_PermutationFolder_Association '/Time_*/Fold_0_Score.mat']);
for i = 1:1000
  ...
  ParCorr_Rand_Fold0(i) = partialcorr(tmp.Predict_Score', EF_Fold0, ...
            [double(Age_Fold0) double(Sex_Fold0) double(Motion_Fold0) double(tmp_Association.Predict_Score')]);
...
Permutation_Fold1_Cell_Association = g_ls([EFAccuracyPrediction_PermutationFolder_Association '/Time_*/Fold_1_Score.mat']);
for i = 1:1000
  ...
  ParCorr_Rand_Fold1(i) = partialcorr(tmp.Predict_Score', EF_Fold1, ...
            [double(Age_Fold1) double(Sex_Fold1) double(Motion_Fold1) double(tmp_Association.Predict_Score')]);
