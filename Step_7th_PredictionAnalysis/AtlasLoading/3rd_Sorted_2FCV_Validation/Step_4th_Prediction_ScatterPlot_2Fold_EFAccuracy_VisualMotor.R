...
PredictionFolder <- paste0(WorkingFolder, '/AtlasLoading_Validation/2Fold_Sort_EFAccuracy_VisualMotor');
...
EFPrediction_Association = paste0(WorkingFolder,
    '/AtlasLoading_Validation/2Fold_Sort_EFAccuracy_Association');
Prediction_Fold0_Association = readMat(paste0(EFPrediction_Association, '/Fold_0_Score.mat'));
PredictScore_Fold0_Association = as.numeric(t(Prediction_Fold0_Association$Predict.Score));
Prediction_Fold1_Association = readMat(paste0(EFPrediction_Association, '/Fold_1_Score.mat'));
PredictScore_Fold1_Association = as.numeric(t(Prediction_Fold1_Association$Predict.Score));
...
# Fold 1
Energy_lm <- lm(PredictScore_Fold1 ~ F1_Exec_Comp_Res_Accuracy + Age + Sex + Motion + PredictScore_Fold1_Association, data = Behavior_Fold1);
...
# Fold 0
Energy_lm <- lm(PredictScore_Fold0 ~ F1_Exec_Comp_Res_Accuracy + Age + Sex + Motion + PredictScore_Fold0_Association, data = Behavior_Fold0);
plotdata <- visreg(Energy_lm, "F1_Exec_Comp_Res_Accuracy", type = "conditional", scale = "linear", plot = FALSE);
...
Fig
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/EFAccuracyPrediction_CorrACC_VisualMotor.tiff', width = 17, height = 15, dpi = 600, units = "cm");
