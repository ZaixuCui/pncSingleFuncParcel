...
PredictionFolder <- paste0(WorkingFolder, '/AtlasLoading_Validation/2Fold_Sort_Age_Association');
...
AgePrediction_VisualMotor = paste0(WorkingFolder,
    '/AtlasLoading_Validation/2Fold_Sort_Age_VisualMotor');
Prediction_Fold0_VisualMotor = readMat(paste0(AgePrediction_VisualMotor, '/Fold_0_Score.mat'));
PredictScore_Fold0_VisualMotor = as.numeric(t(Prediction_Fold0_VisualMotor$Predict.Score));
Prediction_Fold1_VisualMotor = readMat(paste0(AgePrediction_VisualMotor, '/Fold_1_Score.mat'));
...
# Fold 1
Energy_lm <- lm(PredictScore_Fold1 ~ Age + Sex + Motion + PredictScore_Fold1_VisualMotor, data = Behavior_Fold1);
...
# Fold 0
Energy_lm <- lm(PredictScore_Fold0 ~ Age + Sex + Motion + PredictScore_Fold0_VisualMotor, data = Behavior_Fold0);
plotdata <- visreg(Energy_lm, "Age", type = "conditional", scale = "linear", plot = FALSE);
...
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/AgePrediction_CorrACC_Association.tiff', width = 17, height = 15, dpi = 600, units = "cm");
