...
PredictionFolder <- paste0(WorkingFolder, '/AtlasLoading_Validation/2Fold_Sort_EFAccuracy_Association');
...
EFPrediction_VisualMotor = paste0(WorkingFolder,
    '/AtlasLoading_Validation/2Fold_Sort_EFAccuracy_VisualMotor');
Prediction_Fold0_VisualMotor = readMat(paste0(EFPrediction_VisualMotor, '/Fold_0_Score.mat'));
PredictScore_Fold0_VisualMotor = as.numeric(t(Prediction_Fold0_VisualMotor$Predict.Score));
Prediction_Fold1_VisualMotor = readMat(paste0(EFPrediction_VisualMotor, '/Fold_1_Score.mat'));
PredictScore_Fold1_VisualMotor = as.numeric(t(Prediction_Fold1_VisualMotor$Predict.Score));
...
# Fold 1
Energy_lm <- lm(PredictScore_Fold1 ~ F1_Exec_Comp_Res_Accuracy + Age + Sex + Motion + PredictScore_Fold1_VisualMotor, data = Behavior_Fold1);
...
# Fold 0
Energy_lm <- lm(PredictScore_Fold0 ~ F1_Exec_Comp_Res_Accuracy + Age + Sex + Motion + PredictScore_Fold0_VisualMotor, data = Behavior_Fold0);
plotdata <- visreg(Energy_lm, "F1_Exec_Comp_Res_Accuracy", type = "conditional", scale = "linear", plot = FALSE);
...
Fig <- Fig + 
       ...
       scale_y_continuous(limits = c(-2.1, 2.1), breaks = c(-2, -1, 0, 1, 2))
Fig
ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/EFAccuracyPrediction_CorrACC_Association.tiff', width = 17, height = 15, dpi = 600, units = "cm");
