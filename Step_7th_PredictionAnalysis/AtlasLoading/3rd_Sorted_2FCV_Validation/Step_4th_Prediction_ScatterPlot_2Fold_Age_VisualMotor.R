...
PredictionFolder <- paste0(WorkingFolder, '/AtlasLoading_Validation/2Fold_Sort_Age_VisualMotor');
...
AgePrediction_Association = paste0(WorkingFolder,
    '/AtlasLoading_Validation/2Fold_Sort_Age_Association');
Prediction_Fold0_Association = readMat(paste0(AgePrediction_Association, '/Fold_0_Score.mat'));
PredictScore_Fold0_Association = as.numeric(t(Prediction_Fold0_Association$Predict.Score));
Prediction_Fold1_Association = readMat(paste0(AgePrediction_Association, '/Fold_1_Score.mat'));
...
# Fold 1
Energy_lm <- lm(PredictScore_Fold1 ~ Age + Sex + Motion + PredictScore_Fold1_Association, data = Behavior_Fold1);
...
# Fold 0
Energy_lm <- lm(PredictScore_Fold0 ~ Age + Sex + Motion + PredictScore_Fold0_Association, data = Behavior_Fold0);
plotdata <- visreg(Energy_lm, "Age", type = "conditional", scale = "linear", plot = FALSE);
...
Fig <- Fig + 
       ...
       scale_y_continuous(limits = c(5, 22), breaks = c(5, 10, 15, 20))
Fig

ggsave('/data/jux/BBL/projects/pncSingleFuncParcel/Replication/Revision/Figures/AgePrediction_CorrACC_VisualMotor.tiff', width = 17, height = 15, dpi = 600, units = "cm");
