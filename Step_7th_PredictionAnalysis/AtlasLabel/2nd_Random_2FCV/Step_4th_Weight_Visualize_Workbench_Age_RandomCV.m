...
PredictionFolder = [ReplicationFolder '/Revision/PredictionAnalysis/AtlasLabel/'];
...
% Age Prediction %
%%%%%%%%%%%%%%%%%%
w_Brain_Age_lh = zeros(10242, 1);
w_Brain_Age_lh(Index_l) = w_Brain_Age(1:length(Index_l));
w_Brain_Age_rh = zeros(10242, 1);
w_Brain_Age_rh(Index_r) = w_Brain_Age(length(Index_l) + 1:end);
w_Brain_Age_WholeBrain = [w_Brain_Age_lh w_Brain_Age_rh];

V_lh = gifti;
V_lh.cdata = w_Brain_Age_lh;
V_lh_File = [VisualizeFolder '/w_Brain_Age_RandomCV_lh.func.gii'];
save(V_lh, V_lh_File);
pause(1);
V_rh = gifti;
V_rh.cdata = w_Brain_Age_rh;
V_rh_File = [VisualizeFolder '/w_Brain_Age_RandomCV_rh.func.gii'];
save(V_rh, V_rh_File);
% combine 
cmd = ['wb_command -cifti-create-dense-scalar ' VisualizeFolder '/w_Brain_Age_RandomCV' ...
