...
for i = 1:length(BBLID)
    ...
    for j = 1:2
        ...
        cmd = ['echo ' RestingFolder '/' ID_Str '/surf/' hemi{j} '.fs5.sm6.residualised.nii.gz >> ' FilePath];
        system(cmd);
        % session 2, nback
        FilePath = [fMRIListFolder '/' hemi{j} '_sub' ID_Str '_sess2.txt'];
        cmd = ['echo ' nbackFolder '/' ID_Str '/surf/' hemi{j} '.fs5.sm6.residualised.nii.gz >> ' FilePath];
        system(cmd);
        % session 3, emotion identification
        FilePath = [fMRIListFolder '/' hemi{j} '_sub' ID_Str '_sess3.txt'];
        cmd = ['echo ' EmoIdenFolder '/' ID_Str '/surf/' hemi{j} '.fs5.sm6.residualised.nii.gz >> ' FilePath];
        system(cmd);
    end
end
