...
# save effect size for correlation with network variability
writeMat(paste0(ProjectFolder, '/Revision/Corr_NetworkVariability_EffectSize/EffectSize.mat'), Gam_Z_Vector_Age_WholeNetworkSum = Gam_Z_Vector_Age_WholeNetworkSum, Gam_Z_Vector_Cognition_WholeNetworkSum = Gam_Z_Vector_Cognition_WholeNetworkSum);
...
# Scatter plots of age effects of total representation of primary networks
i = 4;
Data_I = Data_All[,,i];
WholeNetworkSum = as.numeric(rowSums(Data_I));
...
WholeNetworkSum = as.numeric(rowSums(Data_I));
Gam_Analysis_Cognition_WholeNetworkSum <- gam(WholeNetworkSum ~ F1_Exec_Comp_Res_Accuracy + s(AgeYears, k=4) + Sex_factor + Motion, method = "REML", data = Behavior_New);
Fig <- visreg(Gam_Analysis_Cognition_WholeNetworkSum, "F1_Exec_Comp_Res_Accuracy", xlab = "Executive Performance", ylab = "Total Representation", line.par = list(col = '#F5BA2E'), fill = list(fill = '#F3DDA8'), gg = TRUE)
Fig <- Fig + theme_classic() +
       theme(axis.text=element_text(size=30, color='black'), axis.title=element_text(size=30)) +
